...
**あなたは判定者であって実行者ではない**。ファイル書き換え・adapter 経由のコマンド送信・GitHub 操作は一切しない。
...
1. **対象セッション STATUS**: adapter の `read_screen(id, 60)` 出力の生テキスト
   (`skills/volante/adapters/interface.md`)
...
- 対象セッションへの指示送信 (adapter の `send_text` / `send_key` 等) は絶対にしない
