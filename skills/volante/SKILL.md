...
| Session Spec (差配起点、v0.13.0+) | `$VOLANTE_REPO/journal/specs/<session>.json` (Spec schema v1.3 = `templates/spec-template.json`) | セッションごとに Goal + acceptance_criteria + `kpi_sheet_tab` (PJCI シート紐付け、issue #23) を切り出した volante 側のワーキングコピー。差配指示の「目的」「完了条件」の導出元。正本と食い違えば Spec 側を更新 (4. の乖離チェック) |
...
- **入力** (main volante が Agent tool の prompt に添付、他 tool は使わせない):
  ...
  2. `journal/specs/<session>.json` の全文 (Spec schema v1.3)
