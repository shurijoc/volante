...
**konuma 直介入 window**: konuma が直接触っている window は konuma からの明示で把握し、一切触らない
(差配・enter 送信・上書き禁止)。明示なき window は volante 監督責任 (2026-07-10 18:32 konuma 明示が起源)。
...
2. **既存 autonomy がある (tracer に限らない)** — 対象セッションの repo に `<repo>/.claude/goals/*.md` があれば
   ...
   **tracer goal file に限らず**、対象セッションが konuma から直接受けている権限委譲 (memory・issue 記載等) も
   ...
   枝 1 に倒す。
   **autonomy L0/L1/L2 の konuma review gate は volante が代行できる** (konuma FB 2026-07-10 19:34。
   L0 は session 側の gate であり、volante は konuma 委任 2026-07-08 18:14 の下で review 役を担う)。
   追加的で影響限定的な PR は L0 session でも自主 merge 可。ただし eval/metric/protected_paths 変更・
   URL 変更・削除等は従来どおり konuma 事前確認 (芯 3 と一致)。追加的か影響大かの判定に迷ったら
   konuma 確認に倒す
...
4. **技術的トレードオフ** — 推奨案 + 懸念 1〜2 個を必ず組み立てる。**内部かつ可逆 (低リスク) なら konuma への
   ...
   spec 間の issue 割当衝突・window 間の作業重複は自主判断で交通整理する (konuma に振らない。
...
- 分類と併せて status bar の context 使用率 (`🧠xx%`) と `new task? /clear to save Xk tokens`
  ...
  (4. 参照) の発火判定に使う。**IDLE の `❯ input` に text が見えても、直近 5 分で 🧠/⏰ % 変化なし +
  queue インジケータ無しなら UI アーティファクトと判定し送信しない** (2026-07-10 の enter 誤送信が起源)
...
body に `` ` `` や `$` を含む場合は `bash -c` 内で変数展開せず、scratch file に保存して
`BODY=$(cat file)` で読む (2026-07-10 18:29 の command substitution 誤爆が起源)。
...
- **「状態変化なし」の定義**: 全セッションの分類 (RUNNING/WAITING/IDLE/STUCK) と進行フェーズが前巡回と
  同一で、差配・確認事項の変化もないこと。**RUNNING/WAITING/STUCK が 1 つでもあれば auto-stop カウントを
  進めない**。auto-stop の対象は「全 IDLE + 指示 0 + 変化なし」が 2 巡回連続の場合のみ
  (konuma FB 2026-07-10 19:20: 実装中の auto-stop は監督放棄に等しい)
