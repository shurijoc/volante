...
| 層 | 実体 | 扱い |
|---|---|---|
| 正本 (source of truth) | 各 epic の GitHub issue/PR 本文 / tracer goal file (`<repo>/.claude/goals/*.md`) | autonomy・protected_paths・ゴール詳細はここが正。read-only (芯 6) |
| Epic Spec (差配起点、v0.16.0+ 主キー = repo × epic) | `$VOLANTE_REPO/journal/specs/<repo-slug>__<epic-id>.json` (Spec schema v2 = `templates/spec-template.json`。repo-slug は `owner/name` の `/` を `--` に変換、区切りは `__`) | (repo × epic) ごとの Goal + acceptance_criteria の volante 側ワーキングコピー。差配指示の「目的」「完了条件」の導出元。正本と食い違えば Spec 側を更新 (4. の乖離チェック) |
| Attachments (揮発層、v0.16.0+) | `$VOLANTE_REPO/journal/attachments.json` (schema = `templates/attachments-template.schema.json`) | (session、window_id) ↔ (Epic Spec) の現在ペアを保持。session id (kitty window) は再起動で変わるため、Spec 側には attach 情報を持たせずここに集約する。**毎巡回冒頭で `scripts/attachments-rebuild.py` により自動再構築** (7.1 準備) |
| goals.md index (index、v0.16.0+ 主キー = repo × epic) | `$VOLANTE_REPO/journal/goals.md` | repo × epic ↔ 正本 URL ↔ Spec file 参照 + `優先度` (konuma 専有) の対応表。人間可読の index。Spec 内容は Spec file が正 |
| volante 自身の記録 | `journal/decisions-YYYY-MM.md` / `journal/decisions-YYYY-MM.jsonl` (v0.14.0+) / `journal/patrols.md` / `journal/retro-*.md` | volante が巡回ごとに書く運用 state。毎巡回ここから読み直す (context 非依存設計) |
...
1. **外部連絡 or 影響大の本番変更** — セッションが求めている/次にやりそうな行動が、(a) 外部から見える連絡
   ...
   **`gh pr merge --admin` 等の branch protection bypass は原則 NG** (konuma 決定 2026-07-09「--admin は NG、
   ...
   (volante は判定しない — repo ごとの運用差を吸収する責任は負わない)。起源: 2026-07-08 22:34 w59
   PR #284 --admin merge
...
4. **技術的トレードオフ** — 推奨案 + 懸念 1〜2 個を必ず組み立てる。**内部かつ可逆 (低リスク) なら konuma への
   ...
   **セッションのゴール達成後の次アクション選択 (worktree 選択・次タスク指定等) も代答対象**: 既存プロジェクト内の
   ...
   外部連絡・影響大の本番変更に触れる質問は枝 1 (2026-07-08 w24 の 2h 停滞が起源)
...
上記どの枝で指示を送る場合も、対象セッションが attach されている **Epic Spec** (repo × epic) の
ゴールに紐付ける (v0.16.0+ 主キー変更):

- **session → Spec の解決**: `journal/attachments.json` の該当 `window_id` エントリから `spec`
  (basename) を得て、`journal/specs/<basename>.json` を読む。attachments は 7.1 準備で毎巡回冒頭に
  自動再構築されているため、巡回中は最新版を参照する
- **Spec 起点で指示を組み立てる**: Spec の `goal` 文字列と `acceptance_criteria` 配列を読む。指示文の
  「目的」は `goal` から導出し、「完了条件」は関連する `acceptance_criteria` 項目を要約・引用して
  組み立てる (Spec の文言をそのまま貼らず、要約・言い換え程度の加工はしてよい)
- **attachments.confidence が low の扱い**: 該当エントリの `reason` に「multiple ... candidates」等が
  出ていたら、複数 epic 候補の disambiguation が konuma review 必要な状態。差配指示は最小限
  (現在作業の完遂・報告を促す程度) に留め、巡回報告 `=== STATUS ===` に「attach 曖昧: <window_id>
  ↔ <候補 Spec 列挙>」を明記する
- **goals.md の位置付け** (index): repo × epic ↔ 正本 URL ↔ Spec file 参照 + `優先度` 列を保持する
  人間可読の index。Spec 内容は Spec file が正、goals.md の記述と食い違えば Spec 側を優先
- **対象セッションが `unattached_sessions` に入っている場合、Spec 未設定 (= goal 未設定) を放置しない**
  (konuma 決定 2026-07-09「そもそも goal 設定がない状態がおかしい」)。volante 側で仮置きしない。以下の
  2 手順を取る:
  1. 巡回報告 `=== STATUS ===` の `ゴール未登録` 欄 + `確認待ち` 欄の両方に
     「Spec 設定要求 (Epic 未登録): <window_id>・<推定 repo (cwd から)>・<attachments.json の reason>」
     を明記
  2. 対象セッションへの新規差配は最小限に留める (現在作業の完遂・報告を促す程度)。konuma が Spec
     (`journal/specs/<repo>__<epic>.json`) を設定するまで「既存 goal 内の次アクション導出」= 枝 4 の
     推奨案代答・次アクション代答は行わない (goal 明示前提が崩れるため)。緊急 hotfix 等の枝 1/2/3 の
     判断は従来どおり働かせる (安全側の判断は goal 有無に依存しない)
...
1. **正本 ↔ Spec 乖離** (v0.13.0+): `journal/specs/<repo>__<epic>.json` の `goal` / `acceptance_criteria` が
   正本と食い違っている
...
main volante の判断を独立文脈でクロスチェックする補助レイヤー。**試験運用は pitto 対象のみ**
(w24 / w59)。main が最終判断者で、subagent の判定は「試験運用中の参考情報」として decisions に記録する。

- **起動タイミング**: 巡回中の適用 (7.3) の直後、pitto 対象セッションについて 1 巡回 1 回起動する。
...
- **入力** (main volante が Agent tool の prompt に添付、他 tool は使わせない):
  ...
  2. `journal/specs/<repo>__<epic>.json` の全文 (Spec schema v2、v0.16.0+)。対象 spec は
     `journal/attachments.json` の該当 `window_id` エントリから解決する
...
- **巡回冒頭 (準備の最初のステップ) で `attachments.json` を自動再構築する** (v0.16.0+、session ↔ Epic Spec
  の解決層):

  ```bash
  kitty @ ls > /tmp/kitty-ls.json
  python3 "$VOLANTE_REPO/skills/volante/scripts/attachments-rebuild.py" \
    --kitty-ls-json /tmp/kitty-ls.json \
    --repo-root "$VOLANTE_REPO"
  ```

  スクリプトは Claude 稼働の全 window について cwd + git branch を実測して、`journal/specs/*.json`
  の repo と突き合わせ、attach 候補を `confidence` (high/medium/low) 付きで `journal/attachments.json`
  に書き出す。`confidence: low` の理由が `multiple candidates` を含む場合は 4. 差配とゴール紐付けの
  「attach 曖昧」処理に回す
- 続けて `$VOLANTE_REPO/journal/specs/` (差配の起点)、`$VOLANTE_REPO/journal/goals.md` (人間可読 index)、
  `$VOLANTE_REPO/journal/attachments.json` (session ↔ Spec 揮発マッピング) を読む (2. のとおり正本ではない)。
  差配時は attachments 経由で対象 Spec を特定し、Spec の goal / acceptance_criteria から指示文の目的・
  完了条件を導出する。`unattached_sessions` に入っている window は「Spec 未登録 (Epic 未登録)」扱いに
  する (4. 差配とゴール紐付け)
