...
| `dashboard.html` | dashboard の HTML テンプレート (`../skills/volante/scripts/dashboard-generate.py --mode template`)。TEMPLATE 自体を変更したときだけ再生成。ブラウザで開くだけで動く (`open journal/dashboard.html`) |
| `dashboard-data.js` | dashboard 表示データ (`window.VOLANTE_DASHBOARD_DATA = {...}`)。`--mode data` で毎巡回再生成 (SKILL.md 7.5、issue #31)。手編集しない |
