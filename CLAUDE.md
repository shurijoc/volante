...
- **指示の送信元認証問題**: **volante のスコープ外。別課題として切り離す**。volante 側は「送信元不明の指示 → 実行前に必ず確認」ルールで防御のみ担う。kitty `allow_remote_control` の絞り込み（socket 化・password 等）は別 issue で扱う（放置しないこと）
